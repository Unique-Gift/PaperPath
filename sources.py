...
if __name__ == "__main__":
    async def test():
        # Test with a well-known open access paper
        test_doi = "10.1038/s41586-021-03819-2"  # AlphaFold paper — should have OA version
        print(f"\n{'='*60}")
        print(f"Testing with DOI: {test_doi}")
        print(f"{'='*60}\n")

        results = await fetch_all_sources(test_doi)

        print("\n--- UNPAYWALL ---")
        if results["unpaywall"]:
            print(f"  Title: {results['unpaywall'].get('title')}")
            print(f"  OA Status: {results['unpaywall'].get('oa_status')}")
            print(f"  Free sources: {len(results['unpaywall'].get('free_sources', []))}")
            print(f"  Response time: {results['unpaywall'].get('response_time_ms')}ms")
        else:
            print("  ❌ No result")

        print("\n--- OPENALEX ---")
        if results["openalex"]:
            print(f"  Title: {results['openalex'].get('title')}")
            print(f"  Authors: {len(results['openalex'].get('authors', []))}")
            print(f"  Corresponding: {results['openalex'].get('corresponding_author', {}).get('name')}")
            print(f"  Response time: {results['openalex'].get('response_time_ms')}ms")
        else:
            print("  ❌ No result")

        print("\n--- SEMANTIC SCHOLAR ---")
        if results["semantic_scholar"]:
            print(f"  Title: {results['semantic_scholar'].get('title')}")
            print(f"  Free PDF: {bool(results['semantic_scholar'].get('free_sources'))}")
            print(f"  Response time: {results['semantic_scholar'].get('response_time_ms')}ms")
        else:
            print("  ❌ No result")

        print(f"\n{'='*60}")
        print(f"Total time: {results['total_response_time_ms']}ms")
        print(f"Sources available: {results['sources_available']}/3")
        print(f"Partial result: {results['partial_result']}")
        print(f"{'='*60}\n")
